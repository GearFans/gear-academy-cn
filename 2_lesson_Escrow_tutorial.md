...
- 我们将检查合约状态是否等于 AwaitingDelivery（为此，我们必须在 EscrowState 枚举上方添加 `#[derive(Debug,PartialEq, Eq)]`：

[代码占位符]

- 然后检查发款人帐户（要获取发送当前正在处理的消息的帐户的 ActorId，我们使用 gstd 库中 msg 模块的 source() 函数）：

[代码占位符]

- 并检查附加资金（要获取附加到正在处理的消息的值，我们使用 gstd 库中 msg 模块的 value() 函数）：
...
让我们通过添加程序元数据来扩展托管程序的功能。我们首先在托管程序的目录中创建一个 escrow-io：

[代码占位符]

该 rust 项目的 `Cargo.toml` 文件将包含以下内容：

[代码占位符]

现在我们可以将 InitEscrow、EscrowAction、EscrowEvent、EscrowState 和 Escrow 移动到那个 rust 项目，并按如下方式定义 `ProgramMetadata`：
...
一旦我们构建了 rust 项目，我们就会有一个名为 `escrow_state.meta.wasm` 的文件，我们可以在 UI 应用程序中使用它来与智能合约进行交互。
...
接下来，我们将使用 `send_with_value` 函数而不是 `send` 函数从买家的账户发送一条消息，因为我们需要发送一条带有资金的消息。然而，在测试节点中，账户余额为零，所以我们必须改变它：

[代码占位符]

为了让事情井井有条，让我们将合约初始化移动到一个名为 `init_escrow(sys:&System)` 的单独函数中：

[代码占位符]

我们可以使用 gtest 库中的 get_program 函数来获取测试函数中的程序。正如从第一课记得的那样，程序是用第一个 id 初始化的。所以，存款测试功能的完整代码如下：
...
3. 接下来，我们回到你在上一课开始编写的 Tamagotchi 合约：
...
-  想计算 Fed、Happy 和 Rested 水平的逻辑。你需要考虑电子宠物最后一次进食、玩耍或睡觉的块数。为此，你可以使用 gstd 库模块 exec 中的函数 block_timestamp()。

4. 现在将你的合约上传到区块链并运行前端应用程序。选择第二课。

现在你可以喂养你的电子宠物，和它一起玩耍，然后让它入睡。
